...
function cakes(recipe, available) {
  let smallestquotient = 1000;

  for (const [recipeKey, recipeValue] of Object.entries(recipe)) {
    let keyFound = false;
    
    for (const [availableKey, availableValue] of Object.entries(available)) {
      if (recipeKey == availableKey) {
        keyFound = true;
        if (availableValue/recipeValue < smallestquotient) {
          smallestquotient = parseInt(availableValue/recipeValue);
        }
      }
    }
